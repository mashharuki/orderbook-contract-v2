...
### Observed Pattern
To safely allow gas to be sponsored or paid by a relayer/executor, `depositFundWithPermit()` intentionally omits a `msg.sender == _owner` check.
...
The off-chain matching engine never constructs same-token routes. The `pause()` function provides the emergency stop mechanism if the executor key is compromised.
