...
| 5 | `test_Sentinel_MECalibrated_ZeroSurplus` | Sentinel | 2-leg. ME calibrated: zero spread on both legs. | `vault.B(taker) = 0`. `vault.A(taker) = 0`. Taker received C. |
| 6 | `test_PerLegFees_DifferentFeesPerLeg` | Per-leg Fees | 2-leg. Leg 1: 5% taker. Leg 2: 10% maker. ME calibrated. | Taker A consumed. Maker1 vault A = 1000. No dust. Solvency. |
...
| 13 | `test_Combined_ThreeLeg_MECalibrated_PerLegFees` | Combined | 3-leg. 2% taker L1, 0% L2, 5% taker L3. ME calibrated zero spread. | `D.bal(taker) = 245e18 - 5%` exactly. No B/C surplus. A fully consumed. |
...
| 2 | `testFuzz_TwoLeg_MECalibrated_PerLegFees` | Fuzz | ME-calibrated 2-leg with random per-leg fees. |
