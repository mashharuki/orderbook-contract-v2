...
contract SeraFuzzTest is TestHelper {
    ...
    function testFuzz_MatchOrderAccountingInvariant(uint256 fillRatioBps) public {
        ...
        Order memory order1 = Order({user: user1, fromToken: address(USDT), toToken: address(SGD), fromAmount: orderAmount, toAmount: orderAmount, feeBps: 0, recipient: user1, expiration: uint48(block.timestamp + 1 days), uuid: 1, routeHash: bytes32(0)});
        Order memory order2 = Order({user: user2, fromToken: address(SGD), toToken: address(USDT), fromAmount: orderAmount, toAmount: orderAmount, feeBps: 0, recipient: user2, expiration: uint48(block.timestamp + 1 days), uuid: 2, routeHash: bytes32(0)});
    ...
    function testFuzz_MinOrderAmountEnforcement(uint256 minAmount, uint256 orderAmount) public {
        ...
        Order memory order1 = Order({user: maker, fromToken: address(USDT), toToken: address(SGD), fromAmount: orderAmount, toAmount: orderAmount, feeBps: 0, recipient: maker, expiration: uint48(block.timestamp + 1 days), uuid: 1, routeHash: bytes32(0)});
        ...
        Order memory order2 = Order({user: taker, fromToken: address(SGD), toToken: address(USDT), fromAmount: orderAmount, toAmount: orderAmount, feeBps: 0, recipient: taker, expiration: uint48(block.timestamp + 1 days), uuid: 2, routeHash: bytes32(0)});
    ...
    function testFuzz_BatchWrapper_ContinueOnFailure(uint256 amount, bool makeSecondExpired) public {
        ...
        Order memory a1 = Order(u1, uint48(block.timestamp + 1 days), 0, u1, address(USDT), address(SGD), amount, amount, bytes32(0), 11);
        Order memory b1 = Order(u2, uint48(block.timestamp + 1 days), 0, u2, address(SGD), address(USDT), amount, amount, bytes32(0), 12);

        Order memory a2 = Order(u1, uint48(block.timestamp + 1 days), 0, u1, address(USDT), address(SGD), amount, amount, bytes32(0), 21);
        Order memory b2 = Order(u2, uint48(block.timestamp + 1 days), 0, u2, address(SGD), address(USDT), amount, amount, bytes32(0), 22);
    ...
    function testFuzz_FOKWrapper_Atomicity(uint256 amount) public {
        ...
        Order memory a1 = Order(u1, uint48(block.timestamp + 1 days), 0, u1, address(USDT), address(SGD), amount, amount, bytes32(0), 31);
        Order memory b1 = Order(u2, uint48(block.timestamp + 1 days), 0, u2, address(SGD), address(USDT), amount, amount, bytes32(0), 32);
        Order memory a2 = Order(u1, uint48(block.timestamp - 1), 0, u1, address(USDT), address(SGD), amount, amount, bytes32(0), 33);
        Order memory b2 = Order(u2, uint48(block.timestamp + 1 days), 0, u2, address(SGD), address(USDT), amount, amount, bytes32(0), 34);
