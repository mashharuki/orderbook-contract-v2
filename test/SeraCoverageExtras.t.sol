...
contract SeraCoverageExtrasTest is TestHelper {
    ...
    function test_coverage_payout_or_deposit_branch() public {
        ...
        Order memory o0 = Order({user: user1, fromToken: address(usdt), toToken: address(sgd), fromAmount: 1000 ether, toAmount: 100 ether, feeBps: 0, recipient: user1, expiration: uint48(block.timestamp + 1 days), uuid: 10, routeHash: bytes32(0)});
        Order memory o1 = Order({user: user2, fromToken: address(sgd), toToken: address(usdt), fromAmount: 100 ether, toAmount: 1000 ether, feeBps: 0, recipient: address(0), expiration: uint48(block.timestamp + 1 days), uuid: 11, routeHash: bytes32(0)});
    ...
    function test_coverage_batcher_error_string_catch() public {
        ...
        matches[0] = MatchData({order0: Order({user: address(1), fromToken: address(2), toToken: address(3), fromAmount: 1, toAmount: 1, feeBps: 0, recipient: address(1), expiration: uint48(block.timestamp + 1), uuid: 1, routeHash: bytes32(0)}), signature0: hex"", matchAmount0: 1, order1: Order({user: address(4), fromToken: address(3), toToken: address(2), fromAmount: 1, toAmount: 1, feeBps: 0, recipient: address(4), expiration: uint48(block.timestamp + 1), uuid: 2, routeHash: bytes32(0)}), signature1: hex"", matchAmount1: 1});
    ...
    function test_coverage_sor_collision_wrap_branch() public {
        ...
        Order memory takerLeg1 = Order({user: taker, fromToken: address(usdt), toToken: address(colliding), fromAmount: 1000 ether, toAmount: 50 ether, feeBps: 0, recipient: address(sera), expiration: uint48(block.timestamp + 1 days), uuid: 21, routeHash: bytes32(0)});
        Order memory makerLeg1 = Order({user: maker, fromToken: address(colliding), toToken: address(usdt), fromAmount: 50 ether, toAmount: 1000 ether, feeBps: 0, recipient: maker, expiration: uint48(block.timestamp + 1 days), uuid: 22, routeHash: bytes32(0)});

        Order memory takerLeg2 = Order({user: taker, fromToken: address(colliding), toToken: address(btc), fromAmount: 50 ether, toAmount: 1 ether, feeBps: 0, recipient: taker, expiration: uint48(block.timestamp + 1 days), uuid: 23, routeHash: bytes32(0)});
        Order memory makerLeg2 = Order({user: mk2, fromToken: address(btc), toToken: address(colliding), fromAmount: 1 ether, toAmount: 50 ether, feeBps: 0, recipient: mk2, expiration: uint48(block.timestamp + 1 days), uuid: 24, routeHash: bytes32(0)});
        ...
        vm.prank(executor);
        sor.executeRoute(matches, _signRoute(takerPK, routeHash, sera), 0);
    ...
    function _routeHash(MatchData[] memory matches) internal pure returns (bytes32) {
        ...
        for (uint256 i = 0; i < matches.length; i++) {
            ...
            bytes32 h = keccak256(abi.encode(ORDER_TYPEHASH, o.user, o.expiration, o.feeBps, o.recipient, o.fromToken, o.toToken, o.fromAmount, o.toAmount, bytes32(0), o.uuid));
