...
contract SeraSOR_Extreme_Test is TestHelper {
    ...
    function test_NoDust_SingleLeg_MixedShares() public {
        ...
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 101, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 101, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_SingleLeg_AllMakerShare() public {
        ...
        MatchData[] memory matches = new MatchData[](1);
        matches[0] = MatchData(t, bytes(""), 1000 ether, m, _signOrder(maker1PK, m, sera), 10 ether);
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 102, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 102, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_ZeroSpread() public {
        ...
        MatchData[] memory matches = new MatchData[](1);
        matches[0] = MatchData(t, bytes(""), 1000 ether, m, _signOrder(maker1PK, m, sera), 10 ether);
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 103, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 103, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_ThreeLegRoute_SpreadEveryLeg() public {
        ...
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 104, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 104, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_FeesAndSpreadWithSentinel() public {
        ...
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 105, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 105, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_WeiLevelRounding_TwoLeg() public {
        ...
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 106, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 106, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_WalletFunded_WithSpread() public {
        ...
        MatchData[] memory matches = new MatchData[](1);
        matches[0] = MatchData(t, bytes(""), 1000 ether, m, _signOrder(maker1PK, m, sera), 10 ether);
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 1000 ether, 107, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 1000 ether, 107, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_MaximumSpread() public {
        ...
        MatchData[] memory matches = new MatchData[](1);
        matches[0] = MatchData(t, bytes(""), 1000 ether, m, _signOrder(maker1PK, m, sera), 10 ether);
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 108, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 108, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_OneSidedSpread() public {
        ...
        MatchData[] memory matches = new MatchData[](1);
        matches[0] = MatchData(t, bytes(""), 1000 ether, m, _signOrder(maker1PK, m, sera), 10 ether);
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 109, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 109, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_TokenConservation_FullRoute() public {
        ...
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 110, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 110, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_PartialFill_TwoRoutes() public {
        ...
        bytes memory sig1 = _signIntent(takerPK, taker, matches1[0].order0.fromToken, matches1[matches1.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 111, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches1, sig1, IntentParams(taker, matches1[0].order0.fromToken, matches1[matches1.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 111, uint48(block.timestamp + 1 days)), uint8(matches1.length * 2 + 1), 0, bytes(""));
        ...
        bytes memory sig2 = _signIntent(takerPK, taker, matches2[0].order0.fromToken, matches2[matches2.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 112, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches2, sig2, IntentParams(taker, matches2[0].order0.fromToken, matches2[matches2.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 112, uint48(block.timestamp + 1 days)), uint8(matches2.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_MaxFeesWithSpread() public {
        ...
        // Near-100% fee (1 unit below BPS_DENOMINATOR), plus spread. Exactly 100% would net the
        // taker 0, which the Finding-1 patch now rejects (ZeroEnvelope needs minOutput>=1 and the
        // envelope floor then reverts a 0-output route); 1 unit below leaves the taker ~dust output
        // so the extreme-fee extraction / solvency / no-dust properties are still exercised.
        Order memory t = _makeOrder(taker, address(usdc), address(eth), 1000 ether, 8 ether, 1);
        t.feeBps = 99_999_999_999_999;
        Order memory m = _makeOrder(maker1, address(eth), address(usdc), 10 ether, 800 ether, 2);
        m.feeBps = 99_999_999_999_999;

        MatchData[] memory matches = new MatchData[](1);
        matches[0] = MatchData(t, bytes(""), 1000 ether, m, _signOrder(maker1PK, m, sera), 10 ether);
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 113, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 113, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
    ...
    function test_NoDust_SharedMaker_TwoLegs() public {
        ...
        bytes memory sig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 114, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, 114, uint48(block.timestamp + 1 days)), uint8(matches.length * 2 + 1), 0, bytes(""));
