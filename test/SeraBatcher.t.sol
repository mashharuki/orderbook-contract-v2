...
contract SeraBatcherTest is TestHelper {
    ...
    function _makePair(uint256 usdtAmt, uint256 sgdAmt, uint256 uuid0, uint256 uuid1) internal view returns (MatchData memory) {
        Order memory o0 = Order({user: maker1, fromToken: address(usdt), toToken: address(sgd), fromAmount: usdtAmt, toAmount: sgdAmt, initialDepositAmount: 0, feeBps: 0, recipient: maker1, expiration: uint48(block.timestamp + 1 days), uuid: uuid0, routeHash: bytes32(0)});
        Order memory o1 = Order({user: maker2, fromToken: address(sgd), toToken: address(usdt), fromAmount: sgdAmt, toAmount: usdtAmt, initialDepositAmount: 0, feeBps: 0, recipient: maker2, expiration: uint48(block.timestamp + 1 days), uuid: uuid1, routeHash: bytes32(0)});
