...
contract SeraSOR_AttackVector_Test is TestHelper {
    ...
    function test_ExpiredDeadline_Reverts() public {
        ...
        uint256 nonce = 999;
        bytes memory sorSig = _signIntent(takerPK, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, 0, 0, taker, 0, nonce, shortDeadline, sera);
        ...
        sor.executeIntent(matches, sorSig, IntentParams(matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, 0, 0, taker, 0, nonce, shortDeadline), uint8(matches.length * 2 + 1), 0, bytes(""));
    }
    ...
    function test_EmptyRoute_Reverts() public {
        ...
        sor.executeIntent(matches, bytes(""), IntentParams(address(0), address(0), 0, 0, taker, 0, block.timestamp, uint48(block.timestamp + 1 days)), uint8(1), 0, bytes(""));
    }
