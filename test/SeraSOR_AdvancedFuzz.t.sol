...
contract SeraSOR_AdvancedFuzz_Test is TestHelper {
    ...
    function testFuzz_StrictSpreadRebateMath(
        ...
        vm.prank(executor);
        sor.executeIntent(matches, sorSig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, 0, block.timestamp, uint48(block.timestamp + 1 days)), type(uint8).max, 0, bytes(""));
    ...
    function testFuzz_WalletFunded_ERC20Boundaries(uint256 initDeposit) public {
        ...
        bytes memory sorSig = _signIntent(takerPK, taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, initDeposit, block.timestamp, uint48(block.timestamp + 1 days), sera);

        vm.prank(owner);
        sera.setSlippageShares(30, 30, 40, 100);

        vm.prank(executor);
        sor.executeIntent(matches, sorSig, IntentParams(taker, matches[0].order0.fromToken, matches[matches.length - 1].order0.toToken, type(uint256).max, 1, taker, initDeposit, block.timestamp, uint48(block.timestamp + 1 days)), type(uint8).max, 0, bytes(""));
    ...
    function testFuzz_PartialFillAccumulation(
        ...
        bytes memory sig1 = _signIntent(takerPK, taker, address(aToken), address(bToken), type(uint256).max, 1, taker, 0, block.timestamp, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(m1, sig1, IntentParams(taker, address(aToken), address(bToken), type(uint256).max, 1, taker, 0, block.timestamp, uint48(block.timestamp + 1 days)), type(uint8).max, 0, bytes(""));
        ...
        bytes memory sig2 = _signIntent(takerPK, taker, address(aToken), address(bToken), type(uint256).max, 1, taker, 0, block.timestamp + 1, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(m2, sig2, IntentParams(taker, address(aToken), address(bToken), type(uint256).max, 1, taker, 0, block.timestamp + 1, uint48(block.timestamp + 1 days)), type(uint8).max, 0, bytes(""));
    ...
    function testFuzz_SpreadConservation(
        ...
        MatchData[] memory matches = new MatchData[](1);
        matches[0] = MatchData(tOrder, bytes(""), tFrom, mOrder, _signOrder(maker1PK, mOrder, sera), mFrom);
        bytes memory sorSig = _signIntent(takerPK, taker, address(aToken), address(bToken), type(uint256).max, 1, taker, 0, block.timestamp, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sorSig, IntentParams(taker, address(aToken), address(bToken), type(uint256).max, 1, taker, 0, block.timestamp, uint48(block.timestamp + 1 days)), type(uint8).max, 0, bytes(""));
    ...
    function testFuzz_EnvelopeGuards_Boundaries(
        ...
        // Expected-revert oracle, in the contract's actual check order:
        //   1. ZeroEnvelope   — either bound zero is rejected up front (Finding 1, _validateAndConsumeIntent)
        //   2. ExcessiveInput — takerInputCost (== takerAmount here) > maxInput (guard checked first)
        //   3. InsufficientOutput — totalTakerOutput (== takerAmount here) < minOutput (guard checked second)
        vm.prank(executor);
        if (maxInput == 0 || minOutput == 0) {
            vm.expectRevert(SeraSOR.ZeroEnvelope.selector);
        } else if (takerAmount > maxInput) {
            vm.expectRevert(SeraSOR.ExcessiveInput.selector);
        } else if (takerAmount < minOutput) {
    ...
    function testFuzz_TwoLegSentinel_RandomPricing(
        ...
        bytes memory sorSig = _signIntent(takerPK, taker, address(aToken), address(cToken), type(uint256).max, 1, taker, 0, block.timestamp, uint48(block.timestamp + 1 days), sera);

        vm.prank(executor);
        sor.executeIntent(matches, sorSig, IntentParams(taker, address(aToken), address(cToken), type(uint256).max, 1, taker, 0, block.timestamp, uint48(block.timestamp + 1 days)), type(uint8).max, 0, bytes(""));
